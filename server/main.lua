...
QBCore.Functions.CreateCallback("garbagejob:server:NextStop", function(source, cb, currentStop, currentStopNum, currLocation)
    ...
    if(math.random(100) >= Config.CryptoStickChance) and Config.GiveCryptoStick then
        Player.Functions.AddItem("empty_weed_bag", 1, true)
        TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items["empty_weed_bag"], 'add')
        TriggerClientEvent('QBCore:Notify', source, "You found a empty weed bag on the floor")
    end
